...
**Current status:** Browser-local capture and targeted link forwarding are implemented; browser-local reset is not. The plugin starts disabled. Settings → UTM Keeper lets administrators enable it and configure supported keys, exact destination hostnames, and retention.
