...
To regenerate the translation template after editing user-facing strings, install [WP-CLI](https://wp-cli.org/) with the `i18n` command and run `composer run pot` from this directory (or run the `wp i18n make-pot` command in `composer.json` directly). This writes `languages/utmkeeperflow.pot` for the `utmkeeperflow` text domain; commit the updated template with string changes. WP-CLI is only needed during development, not at plugin runtime.
